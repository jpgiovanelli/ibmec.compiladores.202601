...
\title{HomeScript: uma Linguagem e um Compilador para Automação Residencial com Geração de linguagem C Para Arduino}
...
\end{lstlisting}
\textcolor{red}{COlocar mais referencias, contextualizar quem está lendo quem mais fez algo similar ou algum contexto historico. falar o que é um compilador.. explicar as nuances de como seria programar. REFERENCIAS!!!!}
...
\textcolor{red}{a seção ficou meio "jogada" nao tem muita ligação com a seção anterior e muito menos com a posterior... ficou tudo vago.}
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=0.95\linewidth]{../img/tela1.png}}
...
\textcolor{red}{falou, falou e nao falou nada.. ficou tudo muito generico.. reescrever}
...
\begin{lstlisting}[style=csmall,caption={Estrutura de Inicialização do Código Gerado para Hardware Integrado.},label={lst:generated}]
#include <Arduino.h>
#include <DHT.h>
...
O frontend foi implementado com React, Vite e Monaco Editor, seguindo o storyboard documentado para a aplicação~\cite{docs-front}. A tela principal funciona como uma IDE: o usuário escreve código HomeScript, escolhe exemplos, compila com botão ou atalho, e visualiza código C, tokens, AST e execução passo a passo. A Fig.~\ref{fig:pipeline} mostra a segunda tela, o construtor visual.

\begin{figure}[htbp]
\centerline{\includegraphics[width=0.95\linewidth]{../img/tela2.png}}
...
\subsection{Mapeamento Físico e Especificação do Hardware Embarcado}

Para validar a corretude e a robustez do pipeline de compilação do HomeScript em escala completa, a lógica de controle correspondente ao cenário integrado foi compilada e embarcada em um ambiente microcontrolado físico baseado na arquitetura ATMega328P (Arduino Uno). Esse procedimento de estresse assegura que as diretivas declarativas geradas pelo software interajam de maneira fidedigna com os registradores e barramentos elétricos de entrada/saída (I/O). 

A topologia do ecossistema físico foi implementada utilizando 9 atuadores e 6 sensores distintos mapeados na matriz de conexões da placa de prototipagem. A Tabela~\ref{tab:hardware_pins} consolida a alocação de pinagens e periféricos utilizados na montagem real.

\textcolor{red}{consertar tabela}

\begin{table}[htbp]
\caption{Mapeamento de Pinos e Periféricos do Ecossistema Físico}
\label{tab:hardware_pins}
\centering
\begin{tabular}{llcp{0.35\linewidth}}
\toprule
\textbf{Identificador} & \textbf{Tipo} & \textbf{Pino} & \textbf{Periférico Físico Simulando} \\
\midrule
\texttt{luz\_sala} & Atuador & 13 & Módulo de Relé / LED Indicador \\
\texttt{luz\_corredor} & Atuador & 12 & LED Indicador de Passagem \\
\texttt{luz\_jardim} & Atuador & 11 & Relé de Iluminação Externa \\
\texttt{ventilador\_quarto} & Atuador & 10 & Driver DC / Cooler de Exaustão \\
\texttt{aquecedor\_quarto} & Atuador & 9 & Módulo de Aquecimento Resistivo \\
\texttt{sirene} & Atuador & 8 & Buzzer Piezoelétrico Ativo \\
\texttt{bomba\_caixa} & Atuador & 7 & Microbomba de Água DC \\
\texttt{portao\_motor} & Atuador & 6 & Servomotor / Motor de Passo \\
\texttt{fita\_led\_garagem} & Atuador & 5 & Barramento de LEDs Digitais \\
\texttt{movimento\_sala\_pin} & Sensor & 2 & Sensor Infravermelho PIR \\
\texttt{porta\_entrada\_pin} & Sensor & 3 & Sensor de Contato Magnético (Reed) \\
\texttt{temperatura\_quarto} & Sensor & 4 & Sensor de Umidade/Temp. DHT11 \\
\texttt{luminosidade\_jardim} & Sensor & A0 & LDR acoplado a Potenciômetro \\
\texttt{nivel\_caixa\_pin} & Sensor & A1 & Potenciômetro de Nível Linear \\
\texttt{distancia\_garagem} & Sensor & A2/A3 & Transdutor Ultrassônico HC-SR04 \\
\bottomrule
\end{tabular}
\end{table}

A validação comportamental do firmware estruturou-se sob constantes dinâmicas e limiares operacionais avaliados pelo loop do microcontrolador. Essas variáveis de parametrização lógica determinavam janelas de histerese e contagens de segurança de eventos no Monitor Serial, conforme apresentado na Tabela~\ref{tab:limiares}.

\textcolor{red}{consertar tabela}

\begin{table}[htbp]
\caption{Parâmetros Operacionais e Constantes de Controle}
\label{tab:limiares}
\centering
\begin{tabular}{lcl}
\toprule
\textbf{Variável do Sistema} & \textbf{Valor / Limiar} & \textbf{Unidade / Função Semântica} \\
\midrule
\texttt{limite\_luz\_jardim} & 450 & Unidades ADC (Escala 0 a 1023) \\
\texttt{temperatura\_max} & 19 & Graus Celsius ($^{\circ}$C) -- Limiar de Resfriamento \\
\texttt{temperatura\_min} & 23 & Graus Celsius ($^{\circ}$C) -- Limiar de Aquecimento \\
\texttt{nivel\_minimo\_caixa} & 380 & Unidades ADC -- Ativação do Fluxo d'Água \\
\texttt{distancia\_abertura} & 4 & Centímetros (cm) -- Escopo Ultrassônico \\
\bottomrule
\end{tabular}
\end{table}

O destrinchamento analítico do fluxo do loop gerado aponta para três acertos fundamentais na tradução de estruturas de controle de fluxo pelo compilador HomeScript:
\begin{itemize}
    \item \textbf{Encapsulamento de Protocolos de Sensores Complexos:} O cálculo de tempo de trânsito de onda do sensor HC-SR04 foi embutido na sub-rotina isolada \texttt{read\_distancia\_garagem\_cm()}, executando modulação de pulsos (\texttt{pulseIn}) limitada por \textit{timeout} de 30 ms para evitar travamentos elétricos. Devido às restrições físicas de placas com mapeamento compartilhado (como o Uno), os pinos \texttt{A2} e \texttt{A3} foram utilizados como I/O digital para as linhas de eco e gatilho do sensor.
    \item \textbf{Laços Aninhados Concorrentes:} A automação da porta de entrada e da garagem confirmou que laços do tipo \texttt{for} (usado na intermitência de 3 pulsos de 250 ms do buzzer da \texttt{sirene}) e do tipo \texttt{while} (usado para executar as 2 tentativas de rotação do \texttt{portao\_motor} com pulsos de 1200 ms) operam sem comprometer as rotinas de amostragem analógica de sensores meteorológicos e volumétricos (\texttt{DHT11} e potenciômetros de nível).
    \item \textbf{Acoplamento Semântico Condicional:} O controle climático executa avaliações aninhadas que impedem a ativação simultânea do atuador de refrigeração e aquecimento, mantendo ambos em nível lógico baixo (\texttt{LOW}) caso as leituras estejam fora das zonas críticas demarcadas.
\end{itemize}

\subsection{Validação Visual do Protótipo}

A Fig.~\ref{fig:arduino_fisico} ilustra o arranjo físico dos transdutores, fiação de interfaceamento de sinais e módulos periféricos montados para a execução do experimento real. Os dados extraídos via comunicação assíncrona UART a 9600 bauds ratificaram que a tradução de conceitos teóricos de compiladores em comandos físicos foi completada com precisão absoluta de eventos.

\begin{figure}[htbp]
\centerline{\includegraphics[width=0.95\linewidth]{../img/sua_foto_do_arduino.jpg}}
\caption{Protótipo físico montado com Arduino Uno, validando a execução integrada do código gerado pelo pipeline HomeScript.}
\label{fig:arduino_fisico}
\end{figure}
...
\textcolor{red}{O ARTIGO ESTA MUITO SEGMENTADO (13 SECOES tentem colocar mau menos tar 4 primeiras em 1 seção com subseçoes... idem para as outras... renomeiem as seç~es com nomes mais macro e coloquem subsecoes mais especificas...pelo menos dupliquem as referencias do artigo...}
